...
var selectionScene = new Phaser.Class({
    ...
    create: function() {
        ...
        var translations = {"en" : ["CHOOSE YOUR AVATAR"], "fr" : ["CHOISIS TON AVATAR"]};

        var titleText = this.add.text(
            screenW * 0.50, screenH * 0.10, translations[_self.lang], {fontSize: 32, color: "white",fontStyle: "bolder", fontFamily: "Mont" }
        ).setOrigin(0.5, 0.65);


        this.playersAvatars = [];
        var playersPos = [
                            {x : 0.14, y : 0.32 }, {x : 0.32, y : 0.32 },{x : 0.50, y : 0.32 }, {x : 0.68, y : 0.32 }, {x : 0.86, y : 0.32 },
                            {x : 0.14, y : 0.67 }, {x : 0.32, y : 0.67 },{x : 0.50, y : 0.67 }, {x : 0.68, y : 0.67 }, {x : 0.86, y : 0.67 }
                         ]
        ...
        function createAvatar(playerObject, i ,pos = {x:0, y : 0})
        {
            ...
            _self.playersAvatars[i].bg.setScale(0.2, 0.3).setInteractive();
            ...
            var translations = {"en" : ["CHOOSE"], "fr" : ["CHOISIR"]};

            _self.playersAvatars[i].btnBg = _self.add.image(0, 0, 'answer_box' );
            _self.playersAvatars[i].btnBg.setScale(0.4, 0.45)
            _self.playersAvatars[i].btnText = _self.add.text(
                0, 0, translations[_self.lang], {fontSize: 19, color: "black",fontStyle: "bolder", fontFamily: "Mont" }
            ).setOrigin(0.5, 0.65);

            _self.playersAvatars[i].bg.on('pointerdown', ()=>{
        ...
        function defineAndRedirect(currentPlayerId)
        {
            ...
            if(!selectedPlayer)
              {
                currentPlayerId = 0;
                selectedPlayer = shuffledPlayers.find(e=>e.id == currentPlayerId);
                let currentPlayerName = window.localStorage.getItem("player_name");

                selectedPlayer.name = currentPlayerName;
              }
